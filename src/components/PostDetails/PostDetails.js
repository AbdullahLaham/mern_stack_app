...
import { getPost, getSearchedPost } from '../../actions/posts';
...
const PostDetails = () => {
  ...
  useEffect(() => {
    // to get the recommended posts that related to this post
    dispatch(getSearchedPost({search: 'none', tags: post?.tags?.join(',') }));
  }, [dispatch]);
  ...
  const recommendedPosts = posts.filter(({_id}) => {
    return _id !== post?._id
  })
  return (
    <Paper style={{padding: '20px', borderRadius: '15px', }} elevation={6} >
      <div className={classes.card} >
        <div className={classes.section}>
          <Typography variant='h6' component='h2' color='textSecondary'>{post?.title}</Typography>
          <Typography variant='body2' color="textSecondary" >{post?.tags?.map((tag) => {
            return `#${tag} `
          })}</Typography>
          <Typography gutterBottom variant='h6' component='p' >{post?.name}</Typography>
          <Typography variant='h6'  >created by: {post?.name}</Typography>
          <Typography variant='body1' component='h2' >{moment(post?.createdAt).fromNow()}</Typography>
            ...
            {recommendedPosts?.map(({title, message, name, likes, selectedFile, _id}) => {
              return (
                <div style={{margin: '20px', cursor: 'pointer',}} onClick={() => {openPost(_id)}} key={_id}>
                  ...
                  <img srs={selectedFile} width="200px"  />
                </div>
              )
            })}
          </div>
